...
// data-sound (clips with an audio track) opts a clip out of the blanket mute;
// playback is still click-to-play, so sound only ever starts from a gesture.
// A sound-flagged hero stays muted inline - unmuted autoplay would be blocked
// and kill the scene-setting clip; its audio plays in the lightbox instead.
vids.forEach(function(v){v.muted=!v.dataset.sound||!!v.closest('.hero-media');v.loop=true;v.setAttribute('playsinline','');
