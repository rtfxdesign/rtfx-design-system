/*{
	"DESCRIPTION": "Void Corridor V2 — raymarched rectangular warp tunnel with spatial audio binding, trail feedback and REVd studio palettes (Transport Beat synced, Anti-Strobe)",
	"CREDIT": "REVd Cycling",
	"ISFVSN": "2",
	"CATEGORIES": [
		"Generator"
	...
	"INPUTS": [
		...
		{ "NAME": "warp", "TYPE": "float", "DEFAULT": 0.5, "MIN": 0.0, "MAX": 1.5, "LABEL": "Tunnel Warp" },
...
void main() {
	...
	for (int i = 0; i < 60; i++) {
		...
		// MID -> per-segment corridor width. Each tunnel segment gets its own phase,
		// so the corridor ripples along its length rather than breathing as one tube.
		float segId     = floor((o.z + r) / mix(0.25, 0.1, si));
		float cellPhase = sin(segId * 1.7 + r * 2.6);
		float ripple    = mAmp * midRipple * wMid * cellPhase;

		vec2 p1 = abs(o.xy) - (mix(0.3, 0.15, si) + ripple * 0.05);
		float dRect = max(p1.x, p1.y);

		float dWave = sin(o.z * 12.0 + r * 2.0) * warp * 0.03;

		x = abs(dRect) - 0.01 + dWave;
		t += max(x, 0.003);
		...
		// Dynamic Color Palette blending color1 & color2
		...
		float spark = 0.5 + 0.5 * sin(segId * 41.7 + r * 9.0);
